...
public final class DataTableUtils {
    ...
    public static <T extends ASchwimmer> ExtendedTableModel resultsEinzelwertung(AWettkampf<T> wk, Altersklasse ak, boolean male, int disz,
        ...
        AWettkampf<T> w = CompetitionUtils.generateEinzelwertungswettkampf(wk, index, removeUnranked);
        if (w == null) {
            return null;
        }
        if (!w.hasSchwimmer()) {
    ...
    private static synchronized <T extends ASchwimmer> void altersklasse(LinkedList<Object[]> result, AWettkampf<T> wk, Altersklasse ak, boolean maennlich,
        ...
        SchwimmerResult<T>[] results = ResultUtils.getResults(wk, ak, maennlich, null, zw);
        if (results.length == 0) {
            return;
        }
    ...
    public static <T extends ASchwimmer> ExtendedTableModel zusatzwertungStartkarten(AWettkampf<T> wk, Feedback fb) {
        if (wk == null) {
            return null;
        }
        HLWListe<T> laufe = wk.getHLWListe();
        ...
        while (li.hasNext()) {
            ...
            result.addLast(startkarteZWToLine(lauf));
        }
     ...
     * @return
     */
    private static <T extends ASchwimmer> Object[] startkarteZWToLine(ZWStartkarte<T> sk) {
    ...
    public static <T extends ASchwimmer> ExtendedTableModel getSchnellsteZeiten(AWettkampf<T> wk, boolean print) {
        ...
        for (int x = 0; x < aks.size(); x++) {
            ...
            for (int y = 0; y < 2; y++) {
                AWettkampf<T> w = CompetitionUtils.generateEinzelwertungswettkampf(wk, x, y == 1);
                if (w != null) {
                    {
