...
abstract class ALauflistenPrinter implements Printer {
    ...
    final class PrintActionListener implements ActionListener {
        ...
        <T extends ASchwimmer> void printLaufliste(OWSelection[] t, boolean withComments) {
            PrintManager.print(getPrintable(getCompetitions(t), getName(), withComments), getName(), true, controller.getWindow());
        }
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            if (wk.isHeatBased()) {
                ISimpleCallback<OWSelection[]> cb = new ISimpleCallback<OWSelection[]>() {
                    ...
                    public void callback(OWSelection[] t) {
                        if (t != null && t.length > 0) {
                            PrintManager.preview(controller.getWindow(), new PPrintableCreator(getCompetitions(t)), getName(), IconManager.getIconBundle(),
                ...
                PrintManager.preview(controller.getWindow(), new PPrintableCreator(new AWettkampf[] { wk }), getName(), IconManager.getIconBundle(),
                        IconManager.getTitleImages());
            }
        }
    }
