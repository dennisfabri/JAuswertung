...
public final class PrintUtils {
    ...
    public static <T extends ASchwimmer> LinkedList<Printable> getEinzelwertungPrintables(AWettkampf<T> wk,
        ...
        for (int x = 0; x < aks.size(); x++) {
            AWettkampf<T> w = CompetitionUtils.generateEinzelwertungswettkampf(wk, x, removeEmpty);
            if (w != null) {
        ...
        for (Wertungsgruppe wg : aks.getWertungsgruppen()) {
            AWettkampf<T> w = CompetitionUtils.generateEinzelwertungswettkampf(wk, wg.getName(), removeEmpty);
            if (w != null) {
                Printable p = getResultsPrintable(w, true, PrintUtils.printPointsInDisciplineResults, removeEmpty,
                        qualification);
                if (p != null) {
    ...
    public static <T extends ASchwimmer> LinkedList<Printable> getEinzelwertungPrintables(AWettkampf<T> wk, int ak,
        ...
        LinkedList<Printable> mp = new LinkedList<Printable>();
        for (int y = 0; y < 2; y++) {
            AWettkampf<T> w = CompetitionUtils.generateEinzelwertungswettkampf(wk, ak, y == 1);
            if (w != null) {
                Printable p = getResultsPrintable(w, true, PrintUtils.printPointsInDisciplineResults, removeEmpty,
                        qualification);
                if (p != null) {
