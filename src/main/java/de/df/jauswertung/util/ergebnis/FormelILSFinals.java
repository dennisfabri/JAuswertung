...
public class FormelILSFinals<T extends ASchwimmer> extends FormelILS<T> {
    ...
    public void setPoints(AWettkampf<T> wk, SchwimmerData<T>[] swimmer, Disziplin d, Hashtable<String, Zielrichterentscheid<T>> zes) {
        ...
        for (int x = 0; x < rounds; x++) {
            ...
            SchwimmerResult<T>[] r = ResultUtils.getResults(wkx, wkx.getRegelwerk().getAk(s.getAKNummer()), male);
        ...
        for (int x = 0; x < swimmer.length; x++) {
            if (sdx == null || comp.compare(sdx, swimmer[x]) != 0) {
                System.out.println("  New group " + xxl.size());
                System.out.println("  " + swimmer[x].getSchwimmer().getName());
                current = new ArrayList<SchwimmerData<T>>();
                xxl.add(current);
                sdx = swimmer[x];
                current.add(sdx);
