...
public final class JResultTable extends JGroupableTable {
    ...
    ASchwimmer[]                  entries              = new ASchwimmer[0];
    double[]                      punkte               = new double[0];
    @SuppressWarnings("rawtypes")
    SchwimmerResult[]             results              = new SchwimmerResult[0];
    @SuppressWarnings("rawtypes")
    AWettkampf                    wk                   = null;
    ...
    private JResultTable(DefaultTableModel dtm, boolean printable) {
        ...
        if (print) {
            setShowRanks(PrintUtils.printRanksInResults);
        } else {
    ...
    public static JResultTable getResultTable(Altersklasse ak, int anzahl, boolean hlw, boolean einzel, boolean print, int qualification, String zwtext) {
        ...
        if (anzahl > 1) {
            ...
            for (int x = 0; x < anzahl; x++) {
                // ColumnGroup disziplinen = new ColumnGroup(I18n.get(
                // "DisciplineNumber", x + 1));
        ...
        // colors[offset + SCORE_OFFSET] = dimmer;
        ...
        AlignmentCellRenderer acr = new AlignmentCellRenderer(new int[] { SwingConstants.RIGHT, SwingConstants.LEFT, SwingConstants.LEFT, SwingConstants.LEFT },
    ...
    public static <T extends ASchwimmer> JResultTable getResultTable(AWettkampf<T> wk, Altersklasse ak, boolean maennlich, boolean print, boolean hlw,
            int qualification) {
        @SuppressWarnings("rawtypes")
        AWettkampf w = wk;
        JResultTable result = getResultTable(ak, ak.getDiszAnzahl(), ak.hasHLW(), w instanceof EinzelWettkampf, print, qualification,
                wk.getRegelwerk().getZusatzwertungShort());
        result.updateResult(wk, ak, maennlich, null, hlw, qualification);
        return result;
    }

    public static <T extends ASchwimmer> JResultTable getResultTable(AWettkampf<T> wk, Altersklasse ak, boolean maennlich, boolean[] select, boolean print,
            boolean hlw, int qualification) {
        @SuppressWarnings("rawtypes")
        AWettkampf w = wk;
        JResultTable result = getResultTable(ak, ak.getDiszAnzahl(), ak.hasHLW(), w instanceof EinzelWettkampf, print, qualification,
                wk.getRegelwerk().getZusatzwertungShort());
    ...
    public <T extends ASchwimmer> void updateResult(AWettkampf<T> w, @SuppressWarnings("hiding") Altersklasse ak, @SuppressWarnings("hiding") boolean maennlich,
            @SuppressWarnings("hiding") boolean[] select, boolean hlw, @SuppressWarnings("hiding") int qualification) {
        synchronized (this) {
            this.wk = w;
    ...
    <T extends ASchwimmer> void updateDiscipline(int x, int y, SchwimmerData<T>[] daten, int offset, boolean choice,
        ...
        setValueAt(PenaltyUtils.getPenaltyMediumText(strafe, daten[x].getSchwimmer().getAK()), y, PREFIX + D_PENALTY_OFFSET + LENGTH * x + offset);
    ...
    class ResultUpdater implements Runnable {
        ...
        @SuppressWarnings("rawtypes")
        public void setData(AWettkampf wk, Altersklasse ak, boolean maennlich, boolean[] select, boolean hlw, int qualification) {
        ...
        public void run() {
            ...
            boolean onlyRanks = FormelManager.getInstance().get(wk.getRegelwerk().getFormelID()).getDataType().equals(DataType.RANK);
            setTimes(!onlyRanks);
            if (print) {
                setShowRanks(PrintUtils.printRanksInResults || onlyRanks);
            } else {
                setShowRanks(onlyRanks);
            }

            results = ResultUtils.getResults(wk, ak, maennlich, select, showZW);
            ...
            entries = new ASchwimmer[results.length];
            punkte = new double[results.length];
            for (int x = 0; x < results.length; x++) {
                entries[x] = results[x].getSchwimmer();
                punkte[x] = results[x].getPoints();
            }

            // Update UI
            updateRowCount(results.length);
            ...
            for (int y = 0; y < rows; y++) {
                ASchwimmer s = entries[y];
                Strafe str = entries[y].getAkkumulierteStrafe(ASchwimmer.DISCIPLINE_NUMBER_SELF);
                if (str.getArt() == Strafarten.AUSSCHLUSS) {
                    ...
                    if (s.isAusserKonkurrenz()) {
                        ...
                        if (results[y].hasKeineWertung()) {
                            setValueAt("-", y, RANK_OFFSET);
                        } else {
                            setValueAt("" + results[y].getPlace(), y, RANK_OFFSET);
                        }
                    }
                }
                ...
                String penalty = PenaltyUtils.getPenaltyMediumText(s.getAkkumulierteStrafe(ASchwimmer.DISCIPLINE_NUMBER_SELF), s.getAK());
                ...
                if (str.getArt() == Strafarten.AUSSCHLUSS) {
                    ...
                    if (results[y].hasKeineWertung()) {
                        ...
                        setValueAt(StringTools.punkteString(results[y].getPoints()), y, SCORE_OFFSET + offset);
                        if (results[y].getPoints() <= 0.005) {
                            setValueAt("", y, DIFF_OFFSET + offset);
                        } else {
                            double d = results[0].getPoints() - results[y].getPoints();
                ...
                @SuppressWarnings("rawtypes")
                SchwimmerData[] daten = results[y].getResults();
                for (int x = 0; x < ak.getDiszAnzahl(); x++) {
                    updateDiscipline(x, y, daten, offset, results[y].getSchwimmer().isDisciplineChosen(x), results[y]);
                }
                if (ak.hasHLW()) {
                    if (showZW) {
                        switch (s.getHLWState()) {
                        case ENTERED:
                            setValueAt(StringTools.punkteString(s.getHLWPunkte(), resultsWithDecimals), y, PREFIX + ak.getDiszAnzahl() * LENGTH + offset);
                            break;
                        case NOT_ENTERED:
                ...
                if (qualification > 0) {
                    int place = results[y].getPlace();
                    boolean isQualified = place <= qualification && place > 0 && !results[y].hasKeineWertung()
                            && !results[y].getSchwimmer().isAusserKonkurrenz() && daten.length == 1;
                    if (isQualified) {
