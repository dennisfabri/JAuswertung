...
import de.df.jauswertung.gui.plugins.heats.*;
...
public class JOWHeatsPlugin extends ANullPlugin {
    ...
    private <T extends ASchwimmer> void updateCompetitors(OWSelection ows) {
        ...
        SchwimmerResult<T>[] result = ResultUtils.getResults(wk, wk.getRegelwerk().getAk(t1.akNummer), t1.male);
        EDTUtils.waitOnEDT();
    ...
    private void printOW(OWSelection sel) {
        ...
        PrintManager.print(mp, I18n.get("AutoprintHeats"), true, getController().getWindow());
    }
