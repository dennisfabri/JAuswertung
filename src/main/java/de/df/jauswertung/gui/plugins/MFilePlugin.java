...
public class MFilePlugin extends ANullPlugin {
    ...
    private void initialize() {
        ...
        menu[1] = new JMenuItem(I18n.get("Open"), IconManager.getSmallIcon("openfile"));
        menu[2] = new JMenuItem(I18n.get("Save"), IconManager.getSmallIcon("savefile"));
        menu[3] = new JMenuItem(I18n.get("SaveAs"), IconManager.getSmallIcon("saveasfile"));

        // init buttons
        buttons = new JButton[3];
        buttons[0] = new JButton(IconManager.getSmallIcon("newfile"));
        buttons[1] = new JButton(IconManager.getSmallIcon("openfile"));
        buttons[2] = new JButton(IconManager.getSmallIcon("savefile"));

        // Set ToolTips
        menu[0].setToolTipText(I18n.getToolTip("NewCompetition"));
        menu[1].setToolTipText(I18n.getToolTip("OpenCompetition"));
        menu[2].setToolTipText(I18n.getToolTip("SaveCompetition"));
        menu[3].setToolTipText(I18n.getToolTip("SaveAsCompetition"));
        buttons[0].setToolTipText(I18n.getToolTip("NewCompetition"));
        buttons[1].setToolTipText(I18n.getToolTip("OpenCompetition"));
        buttons[2].setToolTipText(I18n.getToolTip("SaveCompetition"));

        // set ActionListener
        menu[0].addActionListener(neu);
        menu[1].addActionListener(open);
        menu[2].addActionListener(save);
        menu[3].addActionListener(saveAs);
        buttons[0].addActionListener(neu);
        buttons[1].addActionListener(open);
        buttons[2].addActionListener(save2);

        menu[0].setAccelerator(I18n.getKeyStroke("newfile"));
        menu[1].setAccelerator(I18n.getKeyStroke("openfile"));
        menu[2].setAccelerator(I18n.getKeyStroke("savefile"));
    ...
    private void setSaveIcons(boolean enabled) {
        menu[2].setEnabled(enabled);
        buttons[2].setEnabled(enabled || (core.getFilename() == null));
    }

    @SuppressWarnings("rawtypes")
    void newFile() {
        if (controller.acceptWarning()) {
            JNeuerWettkampf jnw = new JNeuerWettkampf(controller.getWindow());
            jnw.setVisible(true);
            AWettkampf wk = jnw.getWettkampf();
            if (wk != null) {
            ...
            if (controller.acceptWarning()) {
                if (!core.load(name)) {
                    ...
                    if (index >= 0) {
                        ...
                        FileChooserUtils.setBaseDir(dir);
                    }
                }
            }
        }
