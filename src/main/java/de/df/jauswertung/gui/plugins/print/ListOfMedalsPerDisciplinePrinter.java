...
class ListOfMedalsPerDisciplinePrinter implements Printer {
    ...
    Printable getPrintable(AWettkampf<?> wk) {
        ...
        for (int x = 0; x < wk.getRegelwerk().size(); x++) {
            for (int y = 0; y < 2; y++) {
                @SuppressWarnings("rawtypes")
                SchwimmerResult[] result = ResultUtils.getResults(wk, wk.getRegelwerk().getAk(x), y == 1);
    ...
    void preview() {
        PrintManager.preview(controller.getWindow(), new ListOfMedalsPC(), getName(), IconManager.getIconBundle(), IconManager.getTitleImages());
    }
