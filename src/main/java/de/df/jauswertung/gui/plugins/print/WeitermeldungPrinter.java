...
class WeitermeldungPrinter implements Printer {
    ...
    private ExtendedTableModel[] getTableModels(boolean[] selected) {
        AWettkampf wk = CompetitionUtils.convertResultsToMeldung(core.getFilteredWettkampf(), alltimes.isSelected());
        if (!wk.hasSchwimmer()) {
            return null;
        }
    ...
    void print() {
        ...
        if (selection.isAccepted()) {
            PrintManager.print(getPrintable(selection.getSelection()), I18n.get("Registration"), true, controller.getWindow());
        }
    }
