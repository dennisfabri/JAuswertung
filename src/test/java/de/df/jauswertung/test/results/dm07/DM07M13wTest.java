 ...
 */
package de.df.jauswertung.test.results.dm07;

import java.io.FileNotFoundException;

import de.df.jauswertung.daten.*;
import de.df.jauswertung.io.*;
import de.df.jauswertung.misc.MiscImportUtils;
import de.df.jauswertung.test.util.TestUtils;
import de.df.jauswertung.util.ergebnis.*;
import junit.framework.TestCase;

public class DM07M13wTest extends TestCase {

    private static MannschaftWettkampf           wk      = null;

    private static final String                  FILE    = "src/test/resources/competitions/dm07/mannschaft-ak13w.csv";
    ...
    protected void setUp() throws Exception {
        if (TestUtils.INDEPENDENT_INITIALIZATION || (wk == null)) {
            wk = new MannschaftWettkampf(AgeGroupIOUtils.ladeAKs("src/test/resources/rulebooks/DLRG 2007.rwm"),
                    InputManager.ladeStrafen("src/test/resources/rulebooks/DLRG 2007", false));
            data = MiscImportUtils.importFile(wk, FILE, new int[] { 0, 1, 3, 2 });
            results = ResultUtils.getResults(wk, wk.getRegelwerk().getAk(AK), MALE);
        }
    }
    ...
    public void testResults() throws TableFormatException, FileNotFoundException {
        ...
        for (int x = 0; x < results.length; x++) {
            assertEquals(results[x].getSchwimmer().getName(), x + 1, results[x].getSchwimmer().getStartnummer());
        }
    }
