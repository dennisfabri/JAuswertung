...
public class JTeammembersStatusPanel extends JPanel {
    ...
    public JTeammembersStatusPanel() {
        ...
        texts[1] = new JLabel(I18n.get("Input.NamesEntered"));
        texts[2] = new JLabel(I18n.get("Input.Complete"));
        // texts[2] = new JLabel(I18n.get("Input.DidNotStart"));
        // texts[3] = new JLabel(I18n.get("Input.Disqualified"));
    }

    @SuppressWarnings({ "unchecked" })
    public void setData(@SuppressWarnings("rawtypes") AWettkampf wk, String organisation) {
        int[][][] status = TeamUtils.getTeammembersStatus(wk, organisation);
        ...
        for (int x = 0; x < aks.size(); x++) {
            if (SearchUtils.hasSchwimmer(wk, aks.getAk(x))) {
                ...
                for (int y = 0; y < status[x].length; y++) {
                    stati[2 * pos + y][0].setText(format(status[x][y][0]));
                    stati[2 * pos + y][1].setText(format(status[x][y][1] - status[x][y][2]));
                    stati[2 * pos + y][2].setText(format(status[x][y][2]));
                }
