...
final class AresWriterFinals {
    ...
    public static <T extends ASchwimmer> void writeAres(AWettkampf<T>[] wks, String dir) throws IOException {
        writeAKs(wks, dir + File.separator + "LSTCAT.TXT");
        Hashtable<String, Integer> disziplinen = writeLaengen(wks, dir + File.separator + "LSTLONG.TXT");
        LengthUtil.writeStyles(disziplinen, dir + File.separator + "LSTSTYLE.TXT");
        writeNames(wks, dir + File.separator + "Lstconc.TXT");
        writeRecs(wks, dir + File.separator + "LSTREC.TXT");
        writeRaceList(wks, disziplinen, dir + File.separator + "LSTRACE.TXT");
        writeNrList(wks, disziplinen, dir + File.separator + "lstrnum.txt");
        writeRoundList(wks, dir + File.separator + "LSTROUND.TXT");
        writeHeatList(wks, dir + File.separator + "LSTSTART.TXT");
        writeCompetitionInfo(wks, dir + File.separator + "lsttitpr.txt");
    }

    private static void writeAKs(AWettkampf<?>[] wks, String os) throws IOException {
    ...
    private static void writeAKs(Regelwerk aks, StringFileStream ps) throws UnsupportedEncodingException {
    ...
    private static <T extends ASchwimmer> Hashtable<String, Integer> writeLaengen(AWettkampf<T>[] wks, String os) throws IOException {
        try (StringFileStream ps = new StringFileStream(os)) {
    ...
    private static void writeLaengen(Regelwerk aks, StringFileStream ps, Hashtable<String, Integer> disziplinen) throws UnsupportedEncodingException {
        // 0; "25 m" ; 25 ;1
        for (int x = 0; x < aks.size(); x++) {
            ...
            for (int y = 0; y < ak.getDiszAnzahl(); y++) {
                ...
                if (i == null) {
                    ...
                    int anschlaege = discipline.amount();
    ...
    private static <T extends ASchwimmer> void writeRaceList(AWettkampf<T>[] wks, Hashtable<String, Integer> disziplinen, String os)
            throws IOException {
    ...
    private static <T extends ASchwimmer> void writeHeatList(AWettkampf<T>[] wks, String os) throws IOException {
    ...
    private static <T extends ASchwimmer> void writeNrList(AWettkampf<T>[] wks, Hashtable<String, Integer> disziplinen, String os)
            throws IOException {
        // id;event;round ;heat
        // 1 ;1 ;0 ;0 ;
        // 2 ;1 ;0 ;0 ;
    ...
    private static <T extends ASchwimmer> void writeNames(AWettkampf<T>[] wks, String os) throws IOException {
    ...
    private static <T extends ASchwimmer> int writeNames(AWettkampf<T> wk, StringFileStream ps, int offset) {
        for (T t : wk.getSchwimmer()) {
            ps.println(createName(wk, t));
        }
        return wk.getRegelwerk().size() * 2 + offset;
    }

    private static <T extends ASchwimmer> String createName(AWettkampf<T> wk, T t) {
        int sn = getId(t);
        ...
        if (t instanceof Teilnehmer tn) {
            ...
            line += "\"" + cleanupName(t.getName()) + "\";\"\";\"\"";
        }
        line += ";\"" + cleanupName(t.getGliederung()) + "\";\"" + akmw + "\"";
        return line;
    }
    ...
    private static <T extends ASchwimmer> void writeRecs(AWettkampf<T>[] wks, String os) throws IOException {
        try (StringFileStream ps = new StringFileStream(os)) {
    ...
    private static <T extends ASchwimmer> void writeCompetitionInfo(AWettkampf<T>[] wks, String os) throws IOException {
        // event;round;text
    ...
    private static <T extends ASchwimmer> void writeRoundList(AWettkampf<T>[] wks, String os) throws IOException {
        // idLen;idStyle;idRec;abCat;time;name;date;place
        try (StringFileStream ps = new StringFileStream(os)) {
