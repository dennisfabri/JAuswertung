...
public class PRegistrationPlugin extends ANullPlugin {
    ...
    public PRegistrationPlugin() {
        panel = new JPanel(new MigLayout("", "[fill, grow, ::1200lp]", "[fill]"));
    ...
    public void setController(IPluginManager controller, String uid) {
        ...
        pemsp = (PRegistrationInternalMannschaftStaffelPlugin) controller
                .getFeature("de.df.jauswertung.teamteamregistration", uid);
    }
    ...
    void updateDisplay() {
        if (((AWettkampf<?>) core.getWettkampf()) instanceof EinzelWettkampf) {
            panel.removeAll();
            panel.add(peep.getPanel());
        } else {
            panel.removeAll();
            panel.add(pemsp.getPanel());
        }
        panel.updateUI();
    }
}