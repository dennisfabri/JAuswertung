...
public final class StartkartenPrintable extends AComponentMultiOnPagePrintable {
    ...
    static JPanel getPanelI(Startkarte s, boolean etime) {
        ...
        panel.add(new JLabel(I18n.get("Startkarte")), CC.xy(2, 2, "center,center"));
        ...
        panel.add(new JLabel(I18n.get("ProgrammerShortInfo")), CC.xyw(1, 14, 3, "right,center"));
    ...
    private static JPanel createTimepanel(boolean etime) {
        ...
        panel.add(new JLabel(ltext[0]), CC.xy(2, 2, "fill,center"));
        panel.add(new JTimeSpacer(true), CC.xy(4, 2));

        panel.add(new JLabel(ltext[1]), CC.xy(2, 4, "fill,center"));
        panel.add(new JTimeSpacer(true), CC.xy(4, 4));

        panel.add(new JLabel(ltext[2]), CC.xy(2, 6, "fill,center"));
        panel.add(new JTimeSpacer(true), CC.xy(4, 6));

        panel.add(new JLabel(I18n.get("OfficialTime") + ":"), CC.xy(2, 8, "fill,center"));
        ...
        panel.add(new JLabel(I18n.get("Beanstandungen") + ":"), CC.xy(2, 10));
        panel.add(new JLabel(I18n.get("YesNo")), CC.xy(4, 10, "center,fill"));
        panel.add(new JLabel("(Details auf Fehlermeldekarte notieren)") {
            private static final long serialVersionUID = 6458457785694116725L;
        ...
        panel.add(new JLabel(I18n.get("Timekeeper") + ":"), CC.xy(2, 16));
    ...
    private static JPanel createHeader(Startkarte s) {
        ...
        if (s == null) {
            panel.add(new JLabel(I18n.get("AgeGroup") + ":"), CC.xy(2, 4));
            panel.add(new JLabel(I18n.get("Discipline") + ":"), CC.xy(2, 6));
            panel.add(new JLabel(I18n.get("Heat") + ":"), CC.xy(2, 8));
            panel.add(new JLabel(I18n.get("Lane") + ":"), CC.xy(6, 8));
            ...
            panel.add(new JDottingLabel(createNameString(s)), CC.xyw(2, 2, 7));
            panel.add(new JDottingLabel(s.getAK()), CC.xyw(2, 4, 5));
            panel.add(new JLabel(createStartnumberString(s)), CC.xy(8, 4, "right,center"));
            panel.add(new JDottingLabel(s.getDisziplin()), CC.xyw(2, 6, 7));
            if (s.getEvent() > 0) {
                panel.add(new JLabel(I18n.get("EventNr", s.getEvent())), CC.xyw(2, 8, 3, "fill,fill"));
                panel.add(new JLabel(I18n.get("HeatNr", s.getLauf(), 1)), CC.xy(6, 8, "center,fill"));
                panel.add(new JLabel(I18n.get("LaneNumber", s.getBahn())), CC.xy(8, 8, "right,fill"));
            } else {
                panel.add(new JLabel(I18n.get("HeatNr", s.getLauf(), 1)), CC.xyw(2, 8, 3, "fill,fill"));
                panel.add(new JLabel(I18n.get("LaneNumber", s.getBahn())), CC.xyw(6, 8, 3, "fill,fill"));
            }
        }
        return panel;
    }
