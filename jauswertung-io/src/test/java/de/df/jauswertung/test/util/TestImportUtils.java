...
public final class TestImportUtils {

    private TestImportUtils() {
        // Hide constructor
    }

    private static CsvImporter importer = new CsvImporter();

    private static <T extends ASchwimmer> LinkedList<T> importMeldeliste(String name, AWettkampf<T> wk)
    ...
    private static <T extends ASchwimmer> Object[][] importFile(AWettkampf<T> wk, String FILE, int jahr)
            throws TableFormatException, TableEntryException, TableException, IOException {
    ...
    public static <T extends ASchwimmer> Object[][] importFile(AWettkampf<T> wk, String FILE, int[] order)
        ...
        LinkedList<T> ll = TestImportUtils.importMeldeliste(FILE, wk);
        if (ll.isEmpty()) {
            throw new IOException("Could not import file.");
        }
